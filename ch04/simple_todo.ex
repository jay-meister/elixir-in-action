defmodule TodoList do
  ...
  def update_entry(%TodoList{entries: entries} = list, id, fun) when is_map_key(entries, id) do
...
defimpl Collectable, for: TodoList do
  def into(original) do
    into_callback = fn
      list, {:cont, entry} -> TodoList.add_entry(list, entry)
      list, :done -> list
      _list, :halt -> :ok
    end
...
defimpl String.Chars, for: TodoList do
  def to_string(%TodoList{} = list) do
    list.entries
